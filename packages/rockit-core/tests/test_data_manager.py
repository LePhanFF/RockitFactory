"""Tests for SessionDataManager — zip seed, delta merge, dedup, session safety, info."""
...
class TestSessionDataManagerLoad:
    ...
    def test_load_from_zip_seed(self, tmp_path):
        """Extract from zip on first load when CSV doesn't exist."""
        rows = [
            _make_row("2025-03-01T10:00:00.000", session_date="2025-03-01"),
    ...
    def test_load_csv_preferred_over_zip(self, tmp_path):
        """CSV is used when both CSV and zip exist (zip is just the seed)."""
        csv_rows = [
            _make_row("2025-03-01T10:00:00.000", session_date="2025-03-01"),
            _make_row("2025-03-02T10:00:00.000", session_date="2025-03-02"),
            _make_row("2025-03-03T10:00:00.000", session_date="2025-03-03"),
        ]
        zip_rows = [_make_row("2025-03-01T10:00:00.000", session_date="2025-03-01")]

        _write_csv(tmp_path / "NQ_Volumetric_1.csv", _make_csv_content(csv_rows))
        _write_zip(tmp_path / "NQ_Volumetric_1.zip", "NQ_Volumetric_1.csv", _make_csv_content(zip_rows))

        mgr = SessionDataManager(data_dir=tmp_path)
        df = mgr.load("NQ")

        # Should get 3 rows from CSV, not 1 from zip
        assert len(df) == 3
...
class TestSessionDataManagerMerge:
    ...
    def test_merge_adds_new_rows(self, tmp_path):
        ...
        _write_csv(data_dir / "NQ_Volumetric_1.csv", _make_csv_content(local_rows))
    ...
    def test_merge_missing_delta_returns_local(self, tmp_path):
        """When delta file doesn't exist, return local data unchanged."""
        data_dir = tmp_path / "data"
        rows = [_make_row("2025-03-01T10:00:00.000", session_date="2025-03-01")]
    ...
    def test_merge_never_loses_sessions(self, tmp_path):
        ...
        # Local has 2 sessions
        local_rows = [
            _make_row("2025-03-01T10:00:00.000", session_date="2025-03-01"),
            _make_row("2025-03-02T10:00:00.000", session_date="2025-03-02"),
        ]
        # Delta only has 1 session (rolling window moved forward, old session dropped)
        # This shouldn't lose sessions since we concat, but the guard is there
        delta_rows = [
            _make_row("2025-03-02T10:00:00.000", session_date="2025-03-02"),
            _make_row("2025-03-03T10:00:00.000", session_date="2025-03-03"),
        ]
        ...
        # All 3 sessions preserved: local's 2 + delta's 1 new
        assert merged["session_date"].nunique() == 3
